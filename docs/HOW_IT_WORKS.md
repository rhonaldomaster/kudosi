...
4. **Channel Post** - Kudos always post to `#general` (configured via `GENERAL_CHANNEL_ID`)
...
2. Bot opens an interactive modal with:
   ...
   - Category dropdown
   - GIF search (Giphy integration, optional)
3. User fills the form and submits
4. Backend:
   - Posts formatted message to `#general` (`GENERAL_CHANNEL_ID`)
   - Sends DM to each recipient
   - Saves kudos to database
   - Attached image uses selected GIF if provided
